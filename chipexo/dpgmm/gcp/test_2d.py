...
def saveGraph(fn, red, green = None, blue = None):
  ...
  img = numpy.zeros((width,height,3),dtype=numpy.float64)
...
def render(index, ex=''):
  ...
  red = numpy.zeros((width,height),dtype=numpy.float64)
  green = numpy.zeros((width,height),dtype=numpy.float64)
  blue = numpy.zeros((width,height),dtype=numpy.float64)
