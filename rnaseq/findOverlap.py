...
def getNearMaxScore(a,b,covA,covB,fraction,isReciprocal,useStrand):
	...
	while i < len(a) and j < len(b):
		...
		if o > 0 and o*1.0/(int(f1['chromEnd'])-int(f1['chromStart'])) > fraction:
			if (not isReciprocal) or o*1.0/(int(f2['chromEnd'])-int(f2['chromStart'])) > fraction:
				if covB[f2['name']][-1] > ao[f1['name']][0]:
					ao[f1['name']][0] = covB[f2['name']][-1]
				if covB[f2['name']][1] > ao[f1['name']][1]:
					ao[f1['name']][1] = covB[f2['name']][1]
				#ao[f1['name']] = covB[f2['name']]
				if covA[f1['name']][-1] > bo[f2['name']][0]:
					bo[f2['name']][0] = covA[f1['name']][-1]
...
def getCoverage(filename):
	...
	for l in f:
		...
		coverage[tokens[0]] = [float(i) for i in tokens[1:]]
	f.close()	
	return coverage
...
def generateSingleOutput(records,overlaps,coverages,i,infiles,olType):
	...
	for j in range(len(infiles)):
		if j != i:
			out.write("\t%s_count"%(infiles[j].split('/')[-1],))
	out.write("\t%s_maxCov"%(prefix,))
	for j in range(len(infiles)):
		if j != i:
			out.write("\t%s_maxCov"%(infiles[j].split('/')[-1],))
	out.write('\n')
	for r in records:
		out.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t'%(r['chrom'],r['chromStart'],r['chromEnd'],r['name'],r['score'],r['strand'],coverages[r['name']][-1]))
		maxCounts = []
		maxmaxCovs = []
		for j in range(len(infiles)):
			if j != i:
